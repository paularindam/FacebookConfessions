...
for line in lines:
	...
	if "facebook" in values[3]:
		ids = values[3].rsplit('/',1)[1]
		if re.search(r'\d+$', values[3]) is not None and ids.isdigit():
			if "?" in ids:
				ids = ids.split("?")[0]
			...
			if "http:" in values[3]:
				path = values[3].split("http://www").pop()
			elif "https:" in values[3]:
				path = values[3].split("https://www").pop()
